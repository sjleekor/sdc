...
기준: 2026-06-10, PR 4-J active 전환 + daily fact transform 확장(single-input change/vol/yoy/mom, multi-input spread/ratio)까지 완료.
...
| daily fact transform 확장 | (완료) single-input `change_<N>d`/`vol_<N>d`/`yoy`/`mom` + multi-input `spread`/`ratio`(`input_roles` 기반). (남음) seed에 실제 spread feature 등록 — `rate_kr_gov10y` 등 추가 만기 금리 series 수집 선행 필요 |
