...
| 컬럼 | 정의 |
|---|---|
| `ind_ksic_code` | 세션 t 이전(≤ t)에 관측된 마지막 `induty_code`. as-of join on `observed_at` |
| `ind_group` | `definitions/industry_groups.py`의 fold-up(KSIC 2자리 접두 → 그룹 크기 20 미만이면 섹션 → 그래도 미달이면 `OTHER`) |
| `ind_observed_at` | 그 값이 관측된 시각 |
| `ind_is_backcast` | **t < 첫 스냅샷 시각이면 true** — 이 행은 현재 코드 소급이다 |
| `ind_changed_recent_252` | 지난 252세션 안에 코드가 바뀐 적이 있으면 true |
