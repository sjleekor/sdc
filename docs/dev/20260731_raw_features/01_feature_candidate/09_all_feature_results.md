...
25개 family를 전부 넣었다. IC는 |IC|가 가장 큰 primary 셀 기준이고, 부호까지 그대로 적었다.
...
| 5 | `ev_payout_yield` | 주주환원 수익률 | DART | + | + | 40–60일 | +0.0523 | **B** |
| 6 | `px_reversal_5d` | 5일 단기반전 | OHLCV | + | + | 0–3일 | **+0.0540** | **A** |
| 7 | `fin_value_z` | 밸류 종합 z | DART+OHLCV | + | + | 40–60일 | +0.0348 | **B** |
...
| 10 | `ev_net_share_issuance_yoy` | 순주식발행 | DART | − | − | 40–60일 | −0.0083 | **A** |
| — | | | | | | | | |
...
| `ev_net_share_issuance_yoy` | 2016-04-15 | 0.47 | 전기 대비이므로 vintage 2년치 필요 |
| `fin_value_z` | 2019-10-30 | 0.61 | 4개 구성요소 중 2개 이상 유효해야 산출 |
...
**`fin_value_z` — 밸류 종합 · B · IC +0.0348 (40–60일)**
...
0.0722까지 올라간다. **A로 못 간 이유는 통계가 아니라 원천 품질이다** — 정정 비율
...
**`ev_payout_yield` — 주주환원 수익률 · B · IC +0.0523 (40–60일)**

`(현금배당총액 + 자사주매입현금)_ttm / 시가총액`. 배당만 보는 것보다 자사주매입을 합친
쪽이 낫다는 근거(Boudoukh et al. 2007)를 따랐다.

기대 `+` 그대로다. 4개 셀 전부 유의하고 누적 0–120일 IC가 0.0972로 **T2에서 fin_log_mcap
다음으로 크다.** `fin_value_z`와 같은 이유(정정 비율)로 등급이 B에 묶였다.

**`ev_net_share_issuance_yoy` — 순주식발행 · A · IC −0.0083 (40–60일)**
...
기대 `-` 그대로 나왔고 등급 A다. 다만 **IC가 −0.0083으로 25개 중 가장 작다.** 방향은
맞지만 크기는 약하다.
...
| `ev_payout_yield` | cum 0–60 | +0.0765 | +0.93 | +6.93 | 0.1782 | ✗ |
| `fin_value_z` | cum 0–120 | +0.0722 | +1.10 | +4.60 | 0.2317 | ✗ |

`ev_payout_yield`는 IC도 ICIR도 `fin_log_mcap`에 밀리지 않는다. NW t는 오히려 더 크다.
...
T2 상세 조사에서 새로 확인한 것 두 가지는 이 문서의 서술을 보완한다.

- `fin_value_z`에 **결측 처리 버그**가 있다. 구성요소가 없는 종목이 그날 그 시장의
  1분위 값으로 대체돼 전체 행의 19.3%가 "가장 비싼" 끝단에 몰린다
  (`06_grade_a_deep_dive/11_fin_value_z.md` §4). 해당 행을 빼도 decile 스프레드는
  0.76%p → 0.89%p로 오히려 커져 신호 자체는 유지된다.
- `ev_net_share_issuance_yoy`는 **음수 값이 한 건도 없다.** 정의는 순발행인데 실제로는
  총발행을 재고 있고, 값의 96.3%가 정확히 0이다. IC가 25개 중 가장 작은 이유가
  여기 있다(`06_grade_a_deep_dive/09_ev_net_share_issuance_yoy.md` §4).
