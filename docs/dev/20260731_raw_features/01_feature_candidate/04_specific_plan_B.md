...
### 4.5.2 배당 행의 주식 종류 확정과 전순서 (payout_v3 / issuance_v4, 2026-08-15)

**증상.** 같은 코드·같은 입력으로 `feat_event_scan_daily`를 두 번 만들면 결과가 달랐다 —
`ev_net_share_issuance_yoy` 16,790행, `ev_payout_yield` 9,946행,
`issuance_classification_complete` 904행. 즉 이 마트는 재현되지 않았다.

**원인 1 — 주식 종류가 키에 없다.** `dividend_winners`는
`(ticker, bsns_year, row_name)`로 묶고 `(disclosed_date, rcept_no)`로 정렬해 한 행을 고른다.
`stock_knd`가 어디에도 없어서 **보통주와 우선주 행이 같은 파티션에서 경쟁**하고, 같은 공시에서
나왔으니 정렬 키가 완전히 동률이다. 승자는 스캔 순서로 정해진다.

| row_name | 그룹 | 동률 그룹 | 값이 갈리는 그룹 |
|---|---:|---:|---:|
| `주당 현금배당금(원)` | 23,736 | 15,473 | **1,148** |
| `현금배당금총액(백만원)` | 23,737 | 126 | **88** |

이건 재현성만의 문제가 아니다. DPS proxy는 `dps_raw × (발행 − 자기주식)`이므로 **우선주
배당금을 전체 주식수에 곱할 수 있다.**

**판정.** 보통주로 한정한다. 관측된 `stock_knd` 값을 전부 읽고 세 갈래로 나눴다.

| 구분 | 문자열 | 처리 |
|---|---|---|
| 보통주 | `보통주`, `보통주식` | rank 0 — 우선 채택 |
| 종류 미분리 | `-`, 빈 문자열, NULL | rank 1 — 단일 종류 회사이므로 그 값이 곧 보통주 값 |
| 그 외 | `우선주`, `우선주식`, `2우선주`, `제1우선주`, `1우선주`, `종류주식`, `종류주`, `기타주식`, `의결권 있는/없는 주식`, `소액주주`, `대주주` | **제외** |

제외하는 이유는 **어떤 주식수에 곱해야 할지 알 수 없기 때문**이다. 우선주 DPS에 맞는 우선주
발행수를 이 source에서 따로 얻지 못한다. 값을 버리는 비용보다 잘못된 총액을 만드는 비용이
크다. 4.4.2의 정확 일치 원칙을 유지한다 — 부분 일치나 `%우선주%` 같은 패턴을 쓰지 않는다.

**원인 2 — 정렬이 전순서가 아니다.** 종류를 확정해도 같은 공시 안에 같은 종류 행이 여러 개면
동률이 남는다. 승자 선택 네 곳(`dividend_winners`, `winners`, `position_vintage`,
`shares_annual`)에 전순서 tiebreaker를 추가했다. 특히 `position_vintage`는 밀린 보고서를
한꺼번에 내는 회사에서 동률이 난다 — 001470은 2018~2021년 네 개 vintage를 2023-12-12에
동시에 제출했고, 같은 availability에 걸려 어느 것이 뽑히는지가 실행마다 달랐다.

**검증 기준(결과 확인 전 고정).** 같은 코드로 마트를 두 번 만들어 **모든 값 컬럼이 완전히
일치**해야 한다. 하나라도 다르면 재실행하지 않는다.

산식 버전은 `PAYOUT_FEATURE_FORMULA_VERSION = "payout_v3"`,
`EVENT_FEATURE_FORMULA_VERSION = "issuance_v4"`다. v4는 카탈로그 변경이 아니라 tiebreaker
추가지만 값이 움직이므로 올린다.
