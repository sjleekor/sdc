...
### 9.5 내부자·지분구조 — DS002 수집 완료, DS004 미수집

문헌은 일관된다. 내부자 매수는 미래 수익을 예측하고, **매수가 매도보다 정보량이 크다**
(매도는 유동성·분산 목적이 섞인다). 최근 연구는 내부자 거래가 이례현상 롱숏 종목에서 미래
이례현상 수익까지 예측한다고 보고한다.

**DS002 최대주주는 수집했다. 이벤트 시점이 더 깨끗한 DS004는 아직 안 받았다.**

| API | 내용 | 그룹 | 상태 |
|---|---|---|---|
| `elestock` | **임원·주요주주 특정증권등 소유상황보고** | DS004 지분공시 | 미수집 |
| `majorstock` | **대량보유 상황보고**(5% rule) | DS004 지분공시 | 미수집 |
| `hyslr_sttus` | 최대주주 현황 | DS002 정기보고서 | 2015~2025 완료 |
| `hyslr_chg_sttus` | 최대주주 변동 현황 | DS002 정기보고서 | 누적 응답 완료 |

`elestock`이 특히 값이 크다. **보고 의무가 있어 이벤트 시점이 명확하고, PIT 처리가 깨끗하다.**
현재 `dart_filing_receipt_raw`를 이미 수집하고 있으니 수집 파이프라인 패턴도 그대로 쓸 수 있다.

DS002 전체 분석에서는 `own_major_stake` level·change를 Horizon Scan 후보로 골랐다.
`hyslr_chg_sttus`의 control-change 이벤트는 접수 지연 중앙값이 1,092일이라 제외했다.
DS004 후보 `ins_net_buy_90d`·`ins_buy_count_90d`·`own_5pct_holder_chg`는 그대로 남는다.

### 9.6 인적자본·지배구조 — raw 수집 완료, 파생 mart 대기

| API | 내용 |
|---|---|
| `emp_sttus` | **직원 현황** — 직원 수, 평균 근속연수, 1인 평균 급여. 2015~2025 완료 |
| `exctv_sttus` | 임원 현황. 1차 제외 |
| `hmv_audit_indvdl_by_sttus` | 개인별 보수 |
| `accnut_adtor_nm_nd_adt_opinion` | 감사인·감사의견. 2015~2025 씨닝 백필 완료 |

업계에서 workforce analytics(고용 추이·이직률·채용공고)를 대체데이터로 비싸게 사는데,
**한국은 직원 수와 1인당 급여가 정기보고서에 공시된다.** 연 단위·저빈도지만 무료다.

2026-08-27 전체 분석에서 `hc_employee_growth_yoy`와 `hc_revenue_per_employee`를 골랐다.
`hc_avg_pay_growth`는 단위·집계 정규화까지 보류했고, `gov_audit_opinion_flag`는 과거 정정본이
빠진 final-vintage라 `NE`다. →
[`N6 분석`](../02_data_expansion_plan/poc/n6_analysis_20260827.md)
...
| C3 수급·소유·내부자 | 8 | 한국 특화 축. 최대주주 DS002 raw는 수집·후보 선택 완료, 내부자 DS004는 미수집 |
...
| **C7 시장구조·비재무** | **0** | 인적자본 raw는 수집·후보 선택 완료. 아직 Horizon Scan 배선 전 |
| T0 수준 | 12 | |
