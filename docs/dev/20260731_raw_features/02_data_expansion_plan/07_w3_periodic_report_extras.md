...
1. **직원 수 정의** — 정규직만인지 합계인지. 기업마다 구분 방식이 다를 수 있다.
   PoC에서 본 뒤 **하나로 고정**한다
2. **`hc_revenue_per_employee`의 분모 시점** — 직원 수는 기말 기준, 매출은 기간 누적이다.
   기말 기준으로 통일할지 평균을 쓸지 정한다
3. **감사의견 인코딩** — 적정 vs 비적정 이진으로 갈지, 4단계 순서형으로 갈지.
   **권고: 이진.** 비적정이 워낙 드물어 4단계는 표본이 안 나온다
