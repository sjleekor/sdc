...
> **최신 기준일: 2026-08-29.** 과거 날짜별 기록은 보존하고, 실제 현재 상태는 문서 끝의
> `2026-08-29 정리`를 우선한다.
...
>
> **2026-08-28 약관 게이트(K-0c)도 닫았다.** KRX Open API는 비상업·비공개 이용,
> 제3자 제공 금지, 출처 표시, 계약 종료 뒤 이용 중단을 운영 조건으로 고정했다.
> 공공데이터포털 데이터셋 `15094808`은 `이용허락범위 제한 없음`을 다시 확인했다.
...
| 5 | ~~**I7** XBRL fallback 보강~~ → **완료** (2026-08-18). definitions 규칙 40개 + 마트 `fs_basis` 규칙화. **S/P 5% → 79%**, `fin_gross_profitability` 0.03 → 0.81. 남은 건 Phase A/B 재실행뿐 | — |
...
| 14 | ~~**K-0c 약관 게이트** · K-6c KIS 약관~~ | **2026-08-28 완료**. README에 원천별 운영 범위 기록 | — |
...
~~**V 묶음 전에 K-7을 먼저 본다** — `listed_shares` 차분과 합치면 V-1b 전량 재수집이
통째로 불필요해진다.~~ → **전제가 틀렸다 (2026-08-19 실측).** 재료는 실제로 있지만
(`source_open/high/low` 전 연도 100%), **주식수 변동만으로는 분할과 실제 발행을 구분할
수 없다.** 주식수 1.5배 이상 이벤트 1,507건에서 시총이 평균 **2.3배 늘었다** — 액면분할이
아니라 유상증자·전환·합병이라는 뜻이고, ±5거래일 창으로 넓혀도 13.6%만 연속이다.
구분하려면 이벤트 종류(`isu_dcrs_stle`)가 있어야 한다 → **K-7은 V4에, V4는 S-1에 걸린다.**
상세 [`poc/n1_validation.md`](poc/n1_validation.md) §5.
...
- [ ] **N1-9 검증 V1~V8** → `poc/n1_validation.md`
  ...
  - [x] **V2** 원주가/수정주가 비율 (2026-08-19) — **깨끗한 계단 함수다.** 반올림 잡음을
        거르면 1,351종목이 계단 0회, 729종목이 1회, 나머지가 2~10회.
        계단 3,493개 중 1.10배 이상이 2,096개
  - [x] **V3** volume 불일치 (2026-08-19) — **결함을 찾았다.** 같은 날 수집한 행의
        **54%**가 확정 전 naver 값이고(평균 KRX의 98.3%), `--incremental`이
        `MAX(trade_date)` 다음 날부터 읽어서 **영영 교정되지 않는다.** 나중에 백필된
        행은 불일치가 2.94%뿐이다. **정본은 `daily_market_cap.volume`** — 원천이 T+1이라
        구조적으로 확정 전 값을 저장할 수 없다.
        **수정: prod 이벤트에 `PRICE_LOOKBACK_DAYS`를 넘긴다. 요청 수는 그대로다**
  - [x] **V5** 거래대금 근사 (2026-08-19) — **통과.** 세션별 Spearman 순위상관이 전 연도
        평균 **1.0000**, 최솟값 0.9995. **`px_amihud_20d` 재검정 불필요** — 재실행 설정이
        이것 때문에 바뀔 일은 없다
  - [x] **V6** 커버리지 시계열 (2026-08-19) — **통과.** 2,996 세션에서 직전 대비 급락이
        5%·2%·1% 문턱 모두 **0건**. 최악이 KOSPI −0.99%
  - [x] **V8** 이전상장 (2026-08-19) — `daily_ohlcv`에만 있는 행은 **전부 KONEX 구간**.
        `04_w1_pit_universe.md` §3.6
  - [ ] V4 `listed_shares` 급변일 ↔ `dart_capital_change_raw` 이벤트 — **K-7이 여기 걸린다**(§K-7).
        `dart_capital_change_raw`는 prod에 3,144/3,959 법인이 있어 **현재 상장사 범위는 지금 가능**
  - [ ] V7 `listed_shares` vs DART `issued_shares` — **`fin_log_mcap` 개선폭의 상한**.
        상폐 법인분은 S-1 대기
  - 결과: [`poc/n1_validation.md`](poc/n1_validation.md) · 쿼리 [`poc/n1_validation.sql`](poc/n1_validation.sql)
...
- [x] **N2-9 `fin_scan.py` 업종 중립 variant** — **2026-08-27 로컬 실행 완료**
  ...
  - [x] **기존 경로는 SQL이 그대로다**(`industry_view=None`). golden parity 전부 통과.
        기본값이 같은 SQL을 뱉는지 테스트로 고정했다
  - [x] `research/etl/industry.py` — 그룹 결정은 `definitions/industry_groups.py`가 하고
        DuckDB에는 lookup만 넘긴다. **fold-up이 데이터 의존이라 SQL로 옮길 수 없다**
        (2자리 그룹이 20 미만이면 섹션으로, 섹션도 미달이면 `OTHER`로)
...
- [x] **N5-7 (신규) receipt 기반 지분공시 피쳐 구현** — `research/etl/features/filing_activity.py`,
  ...
  - [x] **기존 config는 보존하고 새 스캔 배선을 추가했다** — 확장 config
        `889c3e83…`로 별도 사전등록했고, A/B/AB와 T2 validation까지 끝냈다
...
- [x] **N8-1** → [`poc/n8_employment.md`](poc/n8_employment.md)
  ...
  - [x] ECOS 어댑터가 `ITEM_CODE2`를 지정할 수 있는가 → **이미 된다.** 확장 불필요.
        `endpoint_params`의 `item_code1~4`가 그대로 경로 세그먼트로 나간다.
        **실호출로 확인했고, 여기서 함정이 나왔다** — `item_code2` 없이 `I61BC`만 넣으면
        7개월치가 **14행**으로 온다. **원계열(I28A)과 계절조정(I28B)이 같은 `TIME`에
        서로 다른 값으로 섞여 온다.** 그대로 저장하면 `(series_id, observation_date)`에
        관측이 둘 생기고 나중에 온 쪽이 이긴다
- [x] **N8-2 readiness 창 정렬 — 측정으로 닫았다.** "37개 중 4개만 ready"는
      **시리즈 품질 문제가 아니라 창 문제**였고, 원인이 정확히 둘이다
  - [x] **창 시작일별 실측** (2026-08-12 lake, `readiness_report`)

        | 창 시작 | ready |
        |---|---|
        | 2014-06-02 | **7 / 37** |
        | 2014-07-15 ~ 2017-01-02 | **28 / 37** |
        | **2018-01-02 이후** | **37 / 37** |

  - [x] **원인 ① 한 시리즈가 창 전체를 끌어당긴다.** ~~창이 데이터보다 8세션 먼저 시작한다~~
        → **2026-08-18 정정.** 앞선 판은 제가 임의로 고른 창(2014-06-02)의 산물이었다.
        운영 경로를 그대로 돌리니 **문서의 "4/37"이 정확히 재현된다.**

        `_common_feature_calendars`가 **모든 시리즈에 걸친 `min(available_from_date)`**로
        창을 잡는다. 월간 ECOS 4종(`macro_cpi`·`macro_ppi`·`macro_m2`·
        `macro_consumer_sentiment`)이 **2013-06-20**부터인데 일간 시리즈는 전부
        **2014-06-16**부터다. 그래서 창이 3,394세션(2013-06-20~)이 되고,
        나머지 33개가 **아직 존재하지 않던 257세션**에 대해 `missing_count=257`로 걸린다.
        **ready인 4개가 정확히 그 깊은 이력 4개다.**

        결함이 아니라 이력이 짧은 것뿐이다. **피쳐마다 자기 이력 위에서 판정**하는 게 맞다
  - [x] **원인 ② 딱 하루 — 2017-10-10.** 9개 시리즈가 그날만 NULL이다.
        **2017 추석이 원인이다.** 마지막 세션 09-29 → 다음 세션 10-10 사이가
        **영업일 6일**(10/2·3·4·5·6·9)인데 `max_stale_business_days=5`다.
        **한도를 정확히 하루 넘긴다.** 원본 관측은 그날 정상적으로 있다(KRX 11건) —
        `available_from`이 장 마감 뒤라 그날 시점에 쓸 수 있는 최신 관측이 09-29뿐이다
  - [x] **결론: 시리즈를 늘려도 게이트가 더 어지러워지지 않는다.** N8-2의 전제가 틀렸다.
        실패 30건은 30가지 문제가 아니라 **두 가지 문제**다
  - [x] **결정 둘 다 적용했다** (2026-08-18)
        ① **`max_stale_business_days` 5 → 7.** 표본 전체(2007~2026)에서 최장 휴장을 재보니
        **영업일 6일, 2017년 단 한 번**이다(2위는 5일 = 2025 추석, 기존 값이 여유 0으로 통과).
        7은 관측 최대를 여유 1일로 덮으면서, 수집기가 죽으면 여전히 열흘 안에 NULL이 된다.
        상수 `DAILY_MAX_STALE_BUSINESS_DAYS`로 26곳을 한 곳에서 바꾼다
        ② **readiness를 피쳐별 창으로 바꿨다** — `per_feature_window=True`가 기본.
        **값은 하나도 안 바뀐다** — 게이트 판정 방식이라 `config_hash`와 무관하고 재기준선도
        필요 없다. 자기 이력 안의 구멍은 그대로 잡히므로 **2017-10-10은 계속 보인다**
- [x] **N8-3** `definitions/common_features.py`에 시리즈 3개 추가 + 유닛 테스트 4건.
      **`active=False`로 넣었다** — seed가 활성 시리즈를 바로 수집하기 시작하는데,
      N8-5가 "넣을 것인가"를 아직 안 정했다
- [x] **N8-4** `common seed` → `common sync` 실행, 커버리지 확인 — 세 시리즈 모두
      2000-01~2026-07 **319개월**
- [x] **N8-5 중복 측정 게이트** — 변환 후·availability 정렬 후 실업률·고용률은 통과해
      `active=true`. 취업자 수는 M2 level corr **0.9045**로 `active=false`
- [x] **N8-6** inactive pykrx 폴백 3종은 활성화하지 않기로 끝냈다
...
- [x] **K-0c (신규) 새 원천도 약관 게이트를 통과시킨다** — **2026-08-28 완료.**
  ...
  - [x] 대상 셋 — **KRX Open API · 공공데이터포털 · KIS**
  - [x] 질문 넷 — 비상업 목적 해당 여부 / 장기 보관 / 파생물(피쳐·마트) 지위 / 계약 종료 시 처리
  - [x] **KRX Open API 판정 — 조건부 통과.** 2025-12-26 시행 약관을 다시 확인했다.
        현재 비공개 개인 연구는 비상업 목적에 맞고 제3자에게 결과·데이터를 제공하지 않는다.
        파생 부가정보 생산은 약관 제12조 제3항이 예정하고 있으나 같은 비상업·비공개 조건을
        유지한다. 화면에는 `한국거래소 통계정보` 출처를 표시하고, 이용기간 만료·해지 뒤에는
        수집과 분석 사용을 중단한다. raw·Parquet은 연장 또는 삭제 판단 전까지 compute에서 격리한다
  - [x] **공공데이터포털 판정 — 통과.** `금융위원회_주식시세정보` 데이터셋 `15094808`은
        2026-08-28 현재 무료·`이용허락범위 제한 없음`·개발/운영 자동승인이다. 현재 코드는
        `DATAGO_KEY` 설정만 받고 정기 수집 경로에는 연결하지 않았으며 KRX 값 대조용이다
  - [x] README에 원천별 운영 범위와 코드 MIT 라이선스가 수집 데이터에 적용되지 않는다는 점 기록
...
- [x] **K-6b 실호출 확인 완료** (2026-08-16) —
  ...
  - [x] **K-6c KIS 약관 확인** — **2026-08-28 현재 프로젝트 범위에서 완료.**
        KIS 고객 이용약관 제5조 제3항의 개인 업무 범위와 제3자 제공 금지를 기준으로,
        사용자 본인의 비공개 연구·백테스트에만 쓴다. raw/Parquet 공유, GitHub 업로드,
        다른 사용자가 조회하는 서비스, 종목별 보고서 배포, 상업 모델·투자자문 연결은 하지 않는다.
        계약 종료 뒤 무기한 보관 방침도 현재 확정하지 않는다. 범위가 바뀌면 약관을 다시 확인한다.
        운영 방침은 프로젝트 `README.md`에 기록했다
    - [x] **자격증명 발급 완료** (2026-08-16) — `KIS_APP_KEY`·`KIS_APP_SECRET`·
          `KIS_BASE_URL`·`KIS_TIMEOUT_SECONDS` 설정을 코드와 운영 문서에 연결했다.
          이후 prod에도 반영해 `v0.11.3` KIS 수집을 검증했다
    - [x] 약관 본문 확인 — KIS Developers 고객 이용약관 version 28 확인
...
때 N6 prod run·freshness·Cronicle과 KIS 정기 체인을 따로 확인한다. K-0c는 당시에는
남은 일이었으나 2026-08-28에 완료했다.
...
7. ~~K-0c 약관 게이트~~ — **KIS·KRX Open API·공공데이터포털 모두 README 운영 방침으로 완료**
...
| 날짜 | 내용 |
|---|---|
| 2026-08-28 | **K-0c 완료.** KRX Open API는 비상업·비공개·출처 표시·계약 종료 뒤 이용 중단 조건으로 통과. 공공데이터포털 `15094808`은 무료·이용허락범위 제한 없음을 재확인. 원천별 운영 범위와 코드/데이터 라이선스 경계를 README에 기록 |
| 2026-08-28 | **K-6c KIS 약관 범위 확정.** 비공개 개인 연구·백테스트로 한정하고 제3자 제공·공개·상업 이용을 하지 않는 방침을 README에 기록해 KIS 부분 게이트를 완료 처리 |
...
| 2026-08-18 | **N8-2를 측정으로 닫았다 — 전제가 틀렸다.** "37개 중 4개만 ready"는 시리즈 품질이 아니라 **창** 문제였다. 창 시작일별로 재보니 2014-06-02는 7/37, **2018-01-02 이후는 37/37**이다. 원인이 정확히 둘 — ① common feature가 `daily_ohlcv`보다 **8세션 늦게**(2014-06-16) 시작한다 ② **2017-10-10 하루**, 추석 휴장이 영업일 6일이라 `max_stale_business_days=5`를 **정확히 하루 넘긴다**(원본 관측은 그날 정상이다). **따라서 시리즈를 늘려도 게이트는 더 어지러워지지 않는다.** 곁가지로 ECOS 어댑터는 `item_code2`를 이미 지원하는데, **안 넣으면 원계열과 계절조정이 같은 `TIME`에 섞여 온다**(실호출: 7개월이 14행). N8-3 시리즈 3개는 `active=False`로 넣었다 — N8-5가 "넣을 것인가"를 아직 안 정했다 |
| 2026-08-18 | **N2-9 업종 중립 variant 코드 완료.** 횡단면 파티션 **16곳을 한 문자열로** 묶어 파라미터화했다 — 일부만 업종 기준이 되면 value 합성 점수가 업종 상대와 시장 상대를 섞는다. **기본값은 SQL이 그대로**라 golden parity가 전부 통과한다. 그룹 결정은 `definitions/industry_groups.py`가 하고 DuckDB에는 lookup만 넘긴다 — fold-up이 데이터 의존이라 SQL로 못 옮긴다. **`induty_code`가 없으면 큰 소리로 실패시킨다**: 전부 unknown으로 묶으면 출력이 기존 경로와 똑같아지고 **그 동일함이 발견처럼 읽힌다**. 실행은 D-8 lake 갱신(=sj2) 대기 |
| 2026-08-18 | **I7 완료 — S/P가 살아났다.** 결정 둘을 확정하고 넣었다. ① `fs_basis`를 **규칙별로** 나누되 새 문법 없이 **기존 `fs_div` 관례를 재사용**한다 — `fs_div`가 빈 기존 XBRL 규칙 11개는 `''` 그대로라 parity가 안 깨지고, `CFS`/`OFS`를 명시한 새 규칙만 dimension에서 basis를 읽는다. **이게 첫 시도에서 행이 두 개가 되던 원인이다.** ② 재기준선은 N8-2 결정과 한 라운드로 묶되, **readiness 창 변경은 값이 안 바뀌어** 재실행 대상이 아니다. **실측** — `revenue` 8,103행/약 200종목 → **147,758행/약 2,500종목**, 중복 그룹 **0**, 분기 vintage에서 `revenue` 142,572가 `net_income` 146,774와 사실상 동률. 하류에서 **S/P 5.1% → 79.5%**, **`fin_gross_profitability` 0.03 → 0.79~0.81**, 2019년 `fin_value_z` coverage가 **0.804**로 I1이 깎은 표본을 상쇄한다. `FIN_FEATURE_FORMULA_VERSION` `fin_v3` → **`fin_v4`** — 안 올리면 같은 지문으로 다른 숫자가 나온다. **남은 건 Phase A/B 재실행 하나다** |
| 2026-08-18 | **sj2 복구 — prod 배포와 `flows` 실검증.** 키 5개를 prod `.env`에 넣고 **v0.11.0 → v0.11.1**을 배포했다(prod는 v0.9.10에 멈춰 있었다). compose에 `./state:/state`가 없어 rsync로 같이 올렸다 — 없으면 매 실행이 토큰 재발급 + 알림톡이다. `db init`으로 `collection_slice_state`·`dart_employee_raw`·`dart_governance_raw`·`daily_market_cap` 3컬럼 생성. **prod IP에서 KRX Open API 200** — MDC 차단이 공식 API에 영향 없음을 배포 전에 확인했다. **`flows` 값 검증 통과** — 2026-08-14 4종목 × 3지표 **12개 전부 단위까지 일치**, KIS가 900270의 KRX 공백(07-22~08-14)을 54행으로 메웠다. **실행에서만 나올 버그 둘** — ① 거래정지 종목의 창 밖 응답을 "필드가 사라졌다"로 읽어 5종목 만에 run 중단 ② **tombstone 키에 날짜가 없어** 정지 종목이 7일 차단될 뻔했다(재개를 놓치면 안 되는 바로 그 종목들). 곁가지 정정 — 처음엔 "KIS 공시 지연"으로 봤는데 2시간 뒤 재측정하니 그대로였고, 우리 OHLCV가 답이었다(가격 83,800원 고정 + 거래량 0 = 거래정지). **하루 병행 시행**을 내일 18:30에 걸었다 — KRX Flows가 18:36 시작이라 6분 앞서야 skip을 피한다 |
