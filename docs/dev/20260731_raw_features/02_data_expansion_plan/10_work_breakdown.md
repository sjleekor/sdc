...
- [ ] **K-6f KIS 어댑터 구현** — K-6b 필드 확정 후
  ...
  - [x] **K-6d N7 · N4 조사 완료** (2026-08-16) →
    ...
    - [x] **목적 A에는 지수 구성종목보다 나은 원천이 있다** —
          KRX `업종분류 현황`(`MDC0201020506`) 전종목 CSV.
          화면 다운로드는 **차단 안내가 명시적으로 허용한 경로**다.
          그리고 지수 편입/제외에 섞이는 **신규상장·유동성·방법론 개편 잡음이 없다**
          (`05` §4가 스스로 "측정 3의 정의가 가장 조심할 부분"이라 경고한 그 문제다)
