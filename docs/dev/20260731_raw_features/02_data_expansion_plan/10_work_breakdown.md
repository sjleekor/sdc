...
- [x] **N8-2 readiness 창 정렬 — 측정으로 닫았다.** "37개 중 4개만 ready"는
  ...
  - [x] **원인 ① 창이 데이터보다 8세션 먼저 시작한다.** `daily_ohlcv`는 2014-06-02부터인데
        common feature는 **2014-06-16**부터다. 빠진 8일이 전부 2014-06-02~06-13이고,
        24개 시리즈가 같은 8일 때문에 걸린다. 나머지 `null_count`는 롤링 변환의 워밍업이다
        (`ret_20d`는 앞선 20일이 있어야 계산된다)
  ...
  - [ ] **남은 결정 둘** (둘 다 값이 바뀌므로 재기준선이 필요하다)
        ① `max_stale_business_days`를 6 이상으로 올려 표본 내 최장 휴장(2017 추석)을 덮을 것인가
        ② readiness 창 시작을 common feature 시작일 + 워밍업 뒤로 명시할 것인가
