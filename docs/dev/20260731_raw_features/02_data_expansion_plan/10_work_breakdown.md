...
| 2차 | N4 · N5 | 대기 | N4는 **Open API에 없다** (K-1) |
| 3차 | ledger · N6 · N7 · N8 | 대기 | N7은 **Open API에 없다** (K-1) |
...
> **2026-08-16 차단.** 지수 구성종목은 **Open API 서비스 목록에 없다**(K-1).
> PoC는 스크래핑으로 끝냈지만 **그 경로는 약관 위반이라 실행할 수 없다.**
> K-6에서 대체 경로가 정해지기 전까지 N4-2 이후는 착수하지 않는다.
...
> **2026-08-16 차단.** BPS/PER/PBR/EPS/DIV/DPS는 **Open API 서비스 목록에 없다**(K-1).
> "거의 복사다"라는 전제가 깨졌다 — N1은 Open API로 가고 N7은 갈 곳이 없다.
> K-6에서 대체 경로가 정해지기 전까지 N7-2 이후는 착수하지 않는다.
...
- [ ] **K-6 빈 항목을 어떻게 덮을 것인가.** `flows` · N7 · N4
  ...
  - [ ] **K-6d N7 · N4** — 아직 미조사. `flows`와 달리 **기존 데이터가 없어서** 대체 경로가
        없으면 패키지 자체가 열리지 않는다
