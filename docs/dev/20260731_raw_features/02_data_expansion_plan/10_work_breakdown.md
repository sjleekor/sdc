...
| 1차 | N1 · N2 · N3 | N2 완료 · **N1·N3 차단** | K 묶음 선행 |
| 2차 | N4 · N5 | **N4 재정의** (K-6d) | 지수 구성종목 → 업종분류 현황 스냅샷 |
...
>
> 멈춘 것: **N1-8 · N3-5b · N4 · N7 · S-2 · V-1b**.
> 매일 도는 `flows`·`common krx`·`universe sync`는 **계속 돈다** —
> 셋을 합쳐 하루 약 53요청이고 차단은 백필 6,000요청에서 났다.
> 약관 대상인 건 맞으므로 **한시적**이라는 전제로 둔다(K-6a).
>
> KRX와 무관해 지금 진행 가능한 것: **D-8 → N2-10 V6**, DART 계열(S-1 잔여), 검정 트랙.
...
## KRX Open API 키 없이 진행 가능한 것 (2026-08-16 기준)

인증키가 **8/18~19**에 나올 것으로 본다(광복절 대체공휴일 8/17까지).
그동안 **멈춰 있을 필요가 없는 작업**을 여기 모은다.

| # | 작업 | 왜 지금 되나 | 막고 있는 것 |
|---|---|---|---|
| 1 | **K-6b KIS 확인 4건** | **KIS 키는 이미 있다** — 지금 호출 가능 | — |
| 2 | **K-6f KIS 어댑터** | KIS 키 | K-6b |
| 3 | **S-1 잔여** (financials·share-info·xbrl) | OpenDART만 쓴다. **1,300 법인 누락** | — |
| 4 | **공공데이터포털 활용신청** | KRX 승인과 **무관한 별도 포털** | 사람 |
| 5 | **I7** XBRL fallback 보강 | 순수 코드 + 기존 raw | — |
| 6 | **I6** 정정분 재수집 | OpenDART | — |
| 7 | **D-8 lake 갱신 → N2-10 V6** | 로컬 작업 | — |
| 8 | **O-8/D-5 freshness 알람** | `daily_market_cap` **제외하고 등록하면 된다** | — |
| 9 | 검정 트랙 `0-4` Phase C 인계 판단 | 수집과 독립 | — |

**우선순위 셋.**

1. **K-6b가 가장 값어치 있다.** 키가 있고, 네 질문이 한 번에 풀리고,
   그 답이 K-6f 설계와 **`flows` 커버리지가 5/7인지 6/7인지**를 결정한다
2. **공공데이터포털 신청이 지연 자체를 무력화할 수 있다** —
   `getStockPriceInfo`가 K-2/K-4를 우회한다
3. **S-1 잔여는 N6 선행이고 규모가 확정됐다** — 1,300 법인

**막혀 있는 것**: K-2 · K-4 · K-5 · N1-8 · N3 잔여 · S-2 · K-7 → 전부 인증키 대기.
단 **4번이 되면 K-4까지 앞당겨진다.**
...
- [ ] **K-0b (신규) 운영 지속 여부 결정 — 미결.** 외부 리뷰가 짚은 논리 충돌이다
  - [x] **"하루 38요청이라 계속 돌려도 된다"는 K-0과 충돌한다.**
        약관 제10조 제2호에 **속도 조건이 없다.** 38회와 6,000회 사이에 경계가 없고,
        **호출량은 탐지 위험을 바꿀 뿐 권한을 만들지 않는다**
  - [ ] **안 A — 공식 경로 준비까지 Marketplace 자동화 전면 중단.** 데이터 공백을 감수
  - [ ] **안 B — 계속 돌리되 한시적 위반임을 명시.** 근거는 **"요청량이 작아서"가 아니라
        "공백과 견주어 감수한다"** 여야 한다
  - [ ] **논리적으로 일관된 것은 A다.** B를 택하면 그 판단을 문서에 그대로 적는다
...
- [ ] **K-3 엔드포인트 이용 신청 6건** — 인증키와 **별개 절차다**
  - [ ] **인증키가 아직 안 나왔다** (2026-08-16 기준). 광복절 대체공휴일이 **8/17(월)**까지라
        **8/18~19에 처리될 것으로 본다.** K-2·K-4·N1-8·N3 잔여·S-2가 전부 여기 걸려 있다
  - [ ] **우회 경로를 병행한다** — 공공데이터포털 `getStockPriceInfo`(§K-6d 별건)가
        `mrktTotAmt`·`lstgStCnt`·`trPrc`를 주고 범위 조회를 지원한다.
        **KRX 승인과 무관하게 신청할 수 있고, 되면 K-4를 먼저 시작할 수 있다**
  - [ ] 유가증권 일별매매정보 · 코스닥 일별매매정보
  - [ ] 유가증권 종목기본정보 · 코스닥 종목기본정보
  - [ ] KOSPI 시리즈 일별시세정보 · KOSDAQ 시리즈 일별시세정보
...
- [ ] **K-6f KIS 어댑터 구현** — K-6b 필드 확정 후.
      **어댑터를 쓰는 것보다 감사 모델 정비가 크다**(외부 리뷰)
  - [ ] **전환 조건 6개** — [`flows_alternatives.md`](poc/flows_alternatives.md) §3.1b
    - [ ] **source 전환 cursor** — `Source.KIS`로 바꾸면 source별 `MAX(trade_date)`가 비어
          **증분 시작점이 사라진다**
    - [ ] **종목·날짜·페이지 checkpoint** — 실패 단위가 (날짜, 시장) 공백에서
          **종목별 구멍**으로 바뀐다. 현행 aggregate count + 실패 sample 3건으로는
          **어느 종목·페이지가 빠졌는지 재현 못 한다**
    - [ ] no-data tombstone
    - [ ] **실 HTTP·retry 계수** — `investor_trade_by_stock_daily`는 continuation 페이징이라
          **5,526은 하한이다.** KIS 하루 10,000 quota 감사에도 필요하다
    - [ ] 전역 auth·rate-limit 서킷 브레이커
    - [ ] **source-aware freshness** — `service/freshness.py`가 `Source.KRX` 고정이다
  - [ ] CLI가 `Source.KRX`를 하드코딩한다 (`cli/app.py`)
  ...
  - [x] **K-6d N7 · N4 조사 완료** (2026-08-16) →
    ...
    - [x] **별건 — 공공데이터포털이 K-2를 우회한다.**
          ...
          **출처가 서드파티 문서라 공식 명세 대조가 남았다**
  - [ ] **K-6e 남은 확인 4건** — [`n7_n4_alternatives.md`](poc/n7_n4_alternatives.md) §5
    - [ ] KIS `inquire_price`에 `DIV`(배당수익률)가 있는가
...
- [ ] **S-2 상폐 종목 가격 백필** — `daily_ohlcv`도 `get_active_stocks()`로 대상을 잡으므로
  ...
  - [ ] 실행 — **N3 완료가 선행 조건.** N3가 60/152에서 멈췄으므로 **K-4까지 대기**한다.
