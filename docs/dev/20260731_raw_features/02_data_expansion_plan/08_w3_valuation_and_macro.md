...
> ## ⚠ 2026-08-16 축소 — 아래 A2~A8은 실행하지 않는다
>
> 조사: [`poc/n7_n4_alternatives.md`](poc/n7_n4_alternatives.md) §1 · 판정: K-6d
>
> **A1이 판 근거 셋이 전부 다른 작업으로 이미 귀속돼 있다.**
> `10_known_issues.md`를 다시 읽고 확인한 것이다.
>
> | A1이 든 근거 | 실제 원인 | 어디서 푸는가 |
> |---|---|---|
> | **I1** 29.2%가 1분위에 몰림 | **SQL 버그** — DuckDB `GREATEST`/`LEAST`가 NULL을 건너뛴다 | `fin_scan.py:216-231` 수정. §2에 수정 SQL이 이미 있다 |
> | **I7** `revenue`·`gross_profit` 매핑 | **DART canonical 매핑** | XBRL fallback 보강 (§5.5.3이 이미 결론) |
> | `revision_ratio` → 등급 B | **정정분 재수집** (I6) | DART 재수집 |
>
> **독립 PER/PBR이 셋 중 어디에도 기여하지 않는다.** 코드와 매핑 문제다.
>
> **남는 것은 하나** — I1·I7을 고친 뒤 **canonical 매핑이 맞는지** 독립 원천으로 대조하는 것.
> 이건 진짜 가치지만 **횡단면 문제라 6,000일 이력이 필요 없다.**
>
> **대체 실행**: KIS `inquire_price`(`FHKST01010100`)가 `per`·`pbr`·`eps`·`bps`를 준다.
> 시점당 2,763 호출 ≈ 2.3분, 무료. **선행 조건은 I1·I7·I6 수정이다** —
> 고치기 전에 대조하면 이미 아는 결함을 다시 발견할 뿐이다.
>
> 아래 A2~A8(새 테이블 `daily_market_fundamental` + 6,000 호출 백필)은
> **기록으로 남긴다.** 축소 판단을 되돌릴 근거가 생기면 여기서 재개한다.
