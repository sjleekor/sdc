...
- `universe sync` — sync stock master; `--source krx-openapi` (official, needs `AUTH_KEYS`), `fdr`, or `pykrx`. Prod moved to `--source krx-openapi` on 2026-09-09: `fdr` reads its listing rows out of the `FinanceData/fdr_krx_data_cache` GitHub repo, which stopped publishing after `2026-09-07.csv`, so that path now dies on `HTTP Error 404`.
...
`ALLOW_KRX_SCRAPING` (default `false`) gates the pykrx login path — the collection path KRX restricted this host for. The replacements are wired and verified, so leave it off; set it only for a deliberate one-off comparison. One door is deliberately *not* gated because it still has no replacement in prod: MDC direct (`flows sync`, `common sync --sources krx`). The FDR anonymous door closed itself on 2026-09-09 — `--source fdr` called `data.krx.co.kr` twice per invocation just to read `max_work_dt`, then read its actual rows from a GitHub CSV cache that stopped publishing after `2026-09-07.csv`, so prod's `universe sync` moved to `--source krx-openapi`.

`KIS_APP_KEY`/`KIS_APP_SECRET` (한국투자증권 오픈API) are in **both the local `.env` and prod** as of 2026-08-18; a full-universe `flows sync-kis` ran on prod on 2026-08-19. They serve the KRX-replacement work (K 묶음): KRX restricted this host's IP on 2026-08-16 for a ToS violation, so the scraping path is being replaced by KRX Open API (N1/N3) plus KIS (`flows`). See `docs/operations.md` "KRX 접근 제한". Note the secrets directory holds **connection metadata** (`db_info`, `cronicle_info`) — API keys always go in `.env`, not there.

`AUTH_KEYS` (KRX Open API, comma-separated — two keys as of 2026-08-18) is now in prod too and K-4 shipped: `market_data_krx_openapi` backs both `prices market-cap-backfill` and `universe sync --source krx-openapi`. `DATAGO_KEY` (공공데이터포털) is still local-only and **no adapter reads it** — it survives only as a cross-check source. The KRX keys plus 16 approved endpoints were verified live on 2026-08-18; the response spec lives in `docs/dev/20260731_raw_features/02_data_expansion_plan/poc/krx_open_api.md` §4.1c. Two things the adapter had to handle: a key alone is not enough (each endpoint needs its own 이용 신청, and an unapproved one returns `401 Unauthorized API Call` — distinct from a bad key's `Unauthorized Key`), and one `sto/stk_bydd_trd` call returns a whole market-day including **unadjusted OHLC**, so N1, N3, and K-7 collapse into a single request. That endpoint publishes **T+1**, measured twice on 2026-08-19: a session's rows are not there at 00:43 the next morning but are by 19:31, and the same-day session is never there. So `daily_market_cap` sits one session behind `daily_ohlcv` in the normal state — that is not a fault, and it is why `daily_market_cap` can never satisfy the `max_lag_trading_days=1` staleness check. Anything added to `.env` must get a `Settings` field: pydantic-settings forbids extras, so an undeclared key does not fail at first use — it fails *every* command.
