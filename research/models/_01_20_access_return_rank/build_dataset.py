...
def build_dataset(
    ...
    *,
    ...
    if "fin" in spec.feature_groups:
        lake_tables.append("stock_metric_fact")  # canonical lake (P7)
    if "cf" in spec.feature_groups:
        lake_tables.append("common_feature_daily_fact")  # canonical lake (P8)
    if "ev" in spec.feature_groups:
        lake_tables.append("dart_share_count_raw")  # raw lake (P8)
    register_views(con, config, tables=lake_tables)
