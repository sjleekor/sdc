...
Environment overrides:
  ...
  SDC_RAW_PARQUET_RUNTIME             Default: tools/raw-parquet-exporter/config/local.example.toml.
  SDC_RAW_PARQUET_OUTPUT_ROOT         Default: data_lake/raw_postgres.
  SDC_RAW_PARQUET_SOURCE_NAME         Default: local_mydb.
EOF
}
...
runtime_path="${SDC_RAW_PARQUET_RUNTIME:-tools/raw-parquet-exporter/config/local.example.toml}"
output_root="${SDC_RAW_PARQUET_OUTPUT_ROOT:-data_lake/raw_postgres}"
source_name="${SDC_RAW_PARQUET_SOURCE_NAME:-local_mydb}"
...
for table in "${raw_id_tables[@]}"; do
  export_table "$table" --all-chunks

  if [[ "$validate_samples" == "1" ]]; then
...
for table in "${one_shot_tables[@]}"; do
  export_table "$table"
done

log "Raw Parquet full export completed"
